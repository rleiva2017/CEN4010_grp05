<?php 
	...
	$user_id = 1;
	// connect to database
	$db = mysqli_connect("lamp.cse.fau.edu", "cen4010fal19_g05 ", "tbynSnKO38", "cen4010fal19_g05");
	// get post with id 1 from database
	$post_query_result = mysqli_query($db, "SELECT * FROM posts WHERE id=1");
	while($post = mysqli_fetch_assoc($post_query_result))
	{
	...
	{
		global $db;
		$result = mysqli_query($db, "SELECT username FROM Account WHERE id=" . $id . " LIMIT 1");
		// return the username
